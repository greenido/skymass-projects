...
sm.page("/m", async (ui) => {
  ...
  const manubar = ui.menubar({
    ...
    switch (manubar.didClick) {
      case "help-action": 
        await ui.alert({ text: "This tool gives you the option to see all the NFTs that a specific wallet is holding. \
        You can also suggest to make some as SPAM by clicking on the NFT's row and it \
        will suggest to alchemy.com to check and mark it." });
        break;
      case "help-nft":
